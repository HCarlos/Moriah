<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>{{ config('app.name') }}</title>
    <link href="https://moriah.mx/css/app.css" rel="stylesheet">
    <link href="https://moriah.mx/css/font-awesome.css" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Raleway|PT+Sans+Narrow|Roboto:400,400i,500,500i|Roboto+Mono|Roboto+Condensed|Kaushan+Script&effect=3d-float" rel="stylesheet">

{{--    <link rel="stylesheet" href="{{ asset('/css/ace.css') }}" class="ace-main-stylesheet" id="main-ace-style" />--}}
{{--    <link rel="stylesheet" href="{{ asset('/css/ace-ie.css') }}" />--}}
{{--    --}}
{{--    <link href="{{ asset('assets/css/jquery.dataTables.css') }}" rel="stylesheet">--}}

    <link rel="stylesheet" href="https://moriah.mx/css/ace.css" class="ace-main-stylesheet" id="main-ace-style" />
    <link rel="stylesheet" href="https://moriah.mx/css/ace-ie.css'" />

    <link href="https://moriah.mx/assets/css/jquery.dataTables.css" rel="stylesheet">


    @yield('styles')

{{--    <link href="{{ asset('assets/css/font-awesome.min.css') }}" rel="stylesheet">--}}
{{--    <link rel="stylesheet" href="{{ asset('assets/css/tecnointel.css') }}" />--}}
{{--    <link href="{{ asset('assets/css/sg-01.css') }}" rel="stylesheet">--}}
{{--    <link href="{{ asset('css/my_style_sheet.css') }}" rel="stylesheet">--}}
{{--    <script src="{{ asset('/js/ace-extra.min.js') }}"></script>--}}

    <link href="https://moriah.mx/assets/css/font-awesome.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://moriah.mx/assets/css/tecnointel.css" />
    <link href="https://moriah.mx/assets/css/sg-01.css" rel="stylesheet">
    <link href="https://moriah.mx/css/my_style_sheet.css" rel="stylesheet">
    <script src="https://moriah.mx/js/ace-extra.min.js"></script>

</head>